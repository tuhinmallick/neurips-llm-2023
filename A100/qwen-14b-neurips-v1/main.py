...
model = PeftModel.from_pretrained(model, "quyanh/qwen-14b-neurips-v1")
model.eval()
...
async def process_request(input_data: ProcessRequest) -> ProcessResponse:
    ...
    # prompt = guide.format(user_prompt=input_data.prompt)

    encoded = tokenizer(input_data.prompt, return_tensors="pt")
    
    prompt_length = encoded["input_ids"][0].size(0)
    max_returned_tokens = prompt_length + input_data.max_new_tokens
    assert max_returned_tokens <= MODEL_CONTEXT_LENGTH, (
        max_returned_tokens,
